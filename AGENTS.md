...
5. **Projectiles** — If the special fires a shot, add a kind to `AnimFlags.projectile`, `Projectile.kind`, `spawnFrom`, `FightHooks.spawnProjectile`, and `drawProjectiles`. Measure muzzle offset in world pixels from the fighter origin; do not reuse torso-height defaults.
...
World offset of sprite pixel `(px, py)` is `((px - 80) * scale, (py - 156) * scale)` with `scale = spriteDrawScale(...)`. Projectile muzzle offsets are world pixels.
