...
function qcfPunch(cpu: CpuBrain, me: Fighter): void {
  pushMotion(cpu.plan, [2, 3, 6], rand() < 0.5 ? 'hp' : 'lp', me.facing)
}

function qcfKick(cpu: CpuBrain, me: Fighter): void {
  pushMotion(cpu.plan, [2, 3, 6], rand() < 0.5 ? 'hk' : 'lk', me.facing)
}

function qcbKick(cpu: CpuBrain, me: Fighter): void {
...
const LONG_FIRE: Record<CharId, number> = {
  bob: 0.2,
  ninja: 0.4,
  cyber: 0,
  soldier: 0.45,
...
const LONG_PLAN: Record<CharId, (cpu: CpuBrain, me: Fighter) => void> = {
  bob: (cpu, me) => pushMotion(cpu.plan, [2, 3, 6], 'lp', me.facing),
  ninja: qcfPunch,
  cyber: qcfKick,
  soldier: qcfPunch,
  chainsaw: qcfPunch,
...
export function tickCpu(cpu: CpuBrain, me: Fighter, other: Fighter): VirtualInput {
  ...
  if (dist > 160) {
    const r = rand()
    const fire = LONG_FIRE[I] * sp
    if (fire > 0 && r < fire) LONG_PLAN[I](cpu, me)
  ...
  if (dist > 72) {
    const r = rand()
    if (r < 0.18 * sp && (I === 'ninja' || I === 'toxic')) qcbKick(cpu, me)
  ...
  else if (r < 0.4 * sp && (I === 'cyber' || I === 'soldier' || I === 'chainsaw')) {
    pushMotion(cpu.plan, [2, 3, 6], 'hk', me.facing)
